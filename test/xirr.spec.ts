import { Payment, XirrOptions } from '../src';
import { testCases } from './test_cases';
import { compute } from '../src/index';

describe('xirr', () => {
  const xirr = (payments: Payment[], opts?: XirrOptions) =>
    compute(payments, opts);
  ...
  it('converges for this coronavirus inspired data set', () => {
    const transactions = [
  ...
  it('succeeds even when the N-R algorithm values go into the range (-∞,-1]', () => {
    ...
    const result = xirr(transactions);
    expect(result).toBeCloseTo(-0.83534);
  });
  ...
  it.each(testCases)(
    'tests calculation with given payments',
    (expectedResult, payments) => {
      const result = xirr(payments);
